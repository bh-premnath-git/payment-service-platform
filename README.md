...
then idles. You can customize the hostnames or toggle the PKCS#12 export in the `.env` file. The services continue to ship with
TLS disabled by default; mount paths are provided so you can update the MySQL configuration or WSO2 keystore definitions to use
the shared certificate when needed.
