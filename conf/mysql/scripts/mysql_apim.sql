...
-- ============================================
-- WSO2 IS 7.1.0 Additional Schema
-- ============================================

-- API Resource Management Tables
...
CREATE TABLE IF NOT EXISTS API_RESOURCE_SCOPE (
    ID VARCHAR(255) NOT NULL,
    API_ID VARCHAR(255) NOT NULL,
    TENANT_ID INT NOT NULL,
    NAME VARCHAR(255) NOT NULL,
    ...
    DESCRIPTION VARCHAR(1023),
    PRIMARY KEY (ID),
    UNIQUE KEY API_RESOURCE_SCOPE_NAME_TENANT_ID_CONSTRAINT (API_ID, NAME, TENANT_ID),
    FOREIGN KEY (API_ID) REFERENCES API_RESOURCE(ID) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS API_RESOURCE_ATTRIBUTE (
    ID VARCHAR(255) NOT NULL,
    API_ID VARCHAR(255) NOT NULL,
    TENANT_ID INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS IDN_CONFIG_TYPE (
    ...
    DESCRIPTION VARCHAR(1023),
    PRIMARY KEY (ID),
    UNIQUE KEY CONFIG_TYPE_NAME_CONSTRAINT (NAME)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS IDN_CONFIG_RESOURCE (
    ...
    TYPE_ID VARCHAR(255) NOT NULL,
    PRIMARY KEY (ID),
    UNIQUE KEY CONFIG_RESOURCE_NAME_TYPE_TENANT_CONSTRAINT (NAME, TYPE_ID, TENANT_ID),
    FOREIGN KEY (TYPE_ID) REFERENCES IDN_CONFIG_TYPE(ID) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS IDN_CONFIG_ATTRIBUTE (
    ...
    ATTR_VALUE TEXT,
    PRIMARY KEY (ID),
    UNIQUE KEY CONFIG_ATTRIBUTE_KEY_RESOURCE_CONSTRAINT (ATTR_KEY, RESOURCE_ID),
    FOREIGN KEY (RESOURCE_ID) REFERENCES IDN_CONFIG_RESOURCE(ID) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS IDN_CERT_VALIDATOR (
    ID INTEGER NOT NULL AUTO_INCREMENT,
    TENANT_ID INTEGER NOT NULL,
    NAME VARCHAR(255) NOT NULL,
    TYPE VARCHAR(255) NOT NULL,
    ...
    PRIMARY KEY (ID),
    UNIQUE KEY CERT_VALIDATOR_NAME_TENANT_CONSTRAINT (NAME, TENANT_ID)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS IDN_CERT_VALIDATOR_PROPERTY (
    ID INTEGER NOT NULL AUTO_INCREMENT,
    VALIDATOR_ID INTEGER NOT NULL,
    NAME VARCHAR(255) NOT NULL,
    VALUE TEXT,
    PRIMARY KEY (ID),
    FOREIGN KEY (VALIDATOR_ID) REFERENCES IDN_CERT_VALIDATOR(ID) ON DELETE CASCADE
) ENGINE=InnoDB;

-- Add DEFINED_BY column to IDP_AUTHENTICATOR if it doesn't exist
ALTER TABLE IDP_AUTHENTICATOR
ADD COLUMN IF NOT EXISTS DEFINED_BY VARCHAR(255) DEFAULT 'SYSTEM';

-- Insert default configuration resource types
INSERT IGNORE INTO IDN_CONFIG_TYPE (ID, NAME, DESCRIPTION) VALUES
('176b5d68-3488-4c4e-ab1d-abf91ee05c82', 'CERTIFICATE_VALIDATOR', 'Certificate validator configuration type'),
('e14c1c9f-5b4a-4b76-9a6e-3b5f1c7a8b6d', 'API_RESOURCE', 'API resource configuration type');
